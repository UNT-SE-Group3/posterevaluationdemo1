...
# To install tesseract-ocr in windows machine download exe file from  
...
def extract_text(image):
    ...
    text = image_to_string(image, output_type=Output.STRING)
...
def detect_logo(image):
    ...
    """
    # Detect if there is a logo in the top or top-right corner of the image
    x = int(0.1 * image.shape[0])
    logo_region = image[:x, -1*x:]
